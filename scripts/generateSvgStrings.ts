...
import { filePathToIconCategory, filePathToIconName, filePathToIconSynonyms } from './utils'
import { listFiles } from './_listFiles'
const { sort } = Sort
...
const filesArrayToExportFileContents = (
  kind: 'pop' | 'print' | 'popIndex' | 'printIndex' | 'synonyms' | 'categories' | 'types' = 'pop',
  iconNameFilePathEntries: [string, string][] = [],
): string => {
...
const getIconNameFilePathEntries = async (
  kind: 'pop' | 'print' = 'pop',
): Promise<[string, string][]> => {
  const regex = new RegExp(`.+${kind}\/.+\.svg`, 'gi')
  const files = await listFiles(PATH_PEPICONS + `/export/`, regex)
  const iconNameFilePathEntries: [string, string][] = files.map((filePath) => [
    filePathToIconName(filePath),
