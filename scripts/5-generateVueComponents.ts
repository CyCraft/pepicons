...
export async function generateVueComponents() {
  ...
  for (const type of types) {
    ...
    for (const filename of fs.readdirSync(dir)) {
      ...
      typeIndexContent.push(`export { default as ${componentName} } from './${name}'`)
      icons.push({ name, type, componentName })
    }

    fs.outputFileSync(typeIndexPath, prependWarning(typeIndexContent.join('\n')))
    iconIndexGroupExports.push(`export * as ${type}Icons from './${type}'`)
    iconIndexIndividualExports.push(
      ...icons.map(
        ({ type, componentName, name }) =>
          `export { default as ${componentName} } from './${type}/${name}'`,
      ),
    )
  }
...
import { ${camelCase(name)} } from 'pepicons/dist/${type}.js'
