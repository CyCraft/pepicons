...
export async function generateVueComponents() {
  ...
  const types = ['pop', 'print', 'pencil']
  let iconsIndexContent = ``
  const iconsIndexPath = path.join(PATH_VUE, 'src/icons/index.ts')

  types.forEach((type) => {
    const dir = path.join(PATH_PEPICONS, 'src/icons', type === 'pencil' ? 'print' : type)
...
import { ${camelCase(name)} } from 'pepicons/dist/${type === 'pencil' ? 'print' : type}.esm'
