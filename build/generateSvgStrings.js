...
const filesArrayToExportFileContents = (kind = 'pop', iconNameFilePathEntries = []) => {
  const importTypePepicon =
    kind === 'pop' || kind === 'print' || kind === 'synonyms'
  ...
  if (kind === 'pop' || kind === 'print') {
    ...
    const svgStringProps = iconNameFilePathEntries
      ...
      .join('')
    const _export = `export const ${kind}: { [name in Pepicon]: string } = {\n${svgStringProps}}`
    // all together
    return `${autoGeneratedWarning}\n\n${importTypePepicon}\n\n${_import}\n${_export}\n`
  }

  if (kind === 'types') {
    const iconNamesStringLiteral = iconNameFilePathEntries
      .map(([iconName]) => `'${iconName}'`)
      .join(' | ')
    return `${autoGeneratedWarning}\n\nexport type Pepicon = ${iconNamesStringLiteral}\n`
  }
  ...
  if (kind === 'categories') {
    const categoryProps = iconNameFilePathEntries
      ...
      .join('')
    return `${autoGeneratedWarning}\n\n${importTypePepicon}\n\nexport const categories: { [name in Pepicon]: string } = {\n${categoryProps}}\n`
  }
